#!/usr/bin/env python3
"""Phase 37: batch 50 - document-type normalize mẫu thuế pattern về bucket bảng kê/phụ lục."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase37-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase37-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase37_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    if len(selected) != 50:
        raise RuntimeError(f"Phase37 cần đúng 50 bài, hiện chỉ chọn được {len(selected)}")
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase37-document-type-normalize-bangke-phuluc",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase37Batch50"] = decision["reason"]
