#!/usr/bin/env python3
"""Phase 42: batch 50 - dọn tail đa pool (Thông tư thuế + các residual nhỏ)."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase42-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase42-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase42_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                ...
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase42-thong-tu-balance",
            }

    # B) 1 bài residual kế toán
    ...
    if ke_res:
        decisions[ke_res[0]["href"]] = {
            "kind": "bieu-mau",
            "lv1": "ke-toan",
            "lv2": "mau-bieu-ke-toan",
            "lv3": "mau-so-sach-ke-toan",
            "reason": "phase42-ke-toan-doc-tail",
        }

    # C) 4 bài residual thuế pattern non-generic
    th_res = [
        a for a in sorted(data_articles, key=lambda x: x["href"])
        if a.get("section") == "thu-vien"
        and a.get("libraryKindKey") == "bieu-mau"
    ...
    for a in th_res:
        decisions[a["href"]] = {
            ...
            "reason": "phase42-thue-pattern-tail",
        }

    # D) 1 bài residual nghị định non-generic
    nd_res = [
        a for a in sorted(data_articles, key=lambda x: x["href"])
        if a.get("section") == "thu-vien"
        and a.get("libraryKindKey") == "van-ban"
    ...
    if nd_res:
        decisions[nd_res[0]["href"]] = {
            "kind": "van-ban",
            "lv1": "thue",
            "lv2": "nghi-dinh",
            "lv3": "nghi-dinh-chinh-sach-chung",
            "reason": "phase42-nghi-dinh-tail",
        }

    if len(decisions) != 50:
        raise RuntimeError(f"Phase42 cần đúng 50 bài, hiện build được {len(decisions)}")
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase42Batch50"] = decision["reason"]
