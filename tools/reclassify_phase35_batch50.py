#!/usr/bin/env python3
"""Phase 35: batch 50 - chuẩn hóa nhóm Công văn thuế về nhánh chính sách chung."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase35-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase35-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase35_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    if len(selected) != 50:
        raise RuntimeError(f"Phase35 cần đúng 50 bài, hiện chỉ chọn được {len(selected)}")
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase35-cong-van-generic-normalize",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase35Batch50"] = decision["reason"]
