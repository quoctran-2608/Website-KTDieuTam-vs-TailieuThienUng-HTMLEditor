#!/usr/bin/env python3
"""Phase 34: batch 50 - document-type normalize biểu mẫu thuế về bucket bảng kê/phụ lục."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase34-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase34-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase34_batch50", module_path)
...
def thue_target_lv3(title: str) -> str | None:
    t = title.lower()
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    # Group A: 43 bài thuế có keyword bảng kê/phụ lục nhưng đang để lv3 khác
    rows_a = [
        ...
        and pattern.search(a.get("title") or "")
    ]
    cand_a = []
    for a in rows_a:
        tgt = thue_target_lv3(a["title"])
        cur = a.get("topicLv3Key") or ""
        if tgt and tgt != cur:
            cand_a.append((a["href"], tgt))
    for href, lv3 in sorted(cand_a, key=lambda x: x[0]):
        decisions[href] = {
            "kind": "bieu-mau",
            "lv1": "thue",
            "lv2": "mau-bieu-thue",
            "lv3": lv3,
            "reason": "phase34-thue-specific-refine",
        }

    # Group B: top-up 7 bài kế toán keyword bảng kê/phụ lục -> mau-so-sach-ke-toan
    rows_b = [
        a for a in data_articles
        if a.get("section") == "thu-vien"
        and a.get("libraryKindKey") == "bieu-mau"
        ...
        and pattern.search(a.get("title") or "")
        and (a.get("topicLv3Key") or "") != "mau-so-sach-ke-toan"
    ]
    for a in sorted(rows_b, key=lambda x: x["href"])[:7]:
        decisions[a["href"]] = {
            "kind": "bieu-mau",
            "lv1": "ke-toan",
            "lv2": "mau-bieu-ke-toan",
            "lv3": "mau-so-sach-ke-toan",
            "reason": "phase34-ke-toan-document-type",
        }

    if len(decisions) != 50:
        raise RuntimeError(f"Phase34 cần đúng 50 bài, hiện build được {len(decisions)}")
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase34Batch50"] = decision["reason"]
