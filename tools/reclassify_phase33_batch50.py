#!/usr/bin/env python3
"""Phase 33: batch 50 - chuẩn hóa mẫu thuế dạng bảng kê/phụ lục về bucket tài liệu."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase33-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase33-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase33_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    cand = sorted([a for a in rows if pattern.search(a["title"])], key=lambda x: x["href"])
    selected = cand[:50]
    if len(selected) != 50:
        raise RuntimeError(f"Phase33 cần đúng 50 bài, hiện chọn được {len(selected)}")
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase33-document-type-normalize-bangke-phuluc",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase33Batch50"] = decision["reason"]
