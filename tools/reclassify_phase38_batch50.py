#!/usr/bin/env python3
"""Phase 38: batch 50 - chuẩn hóa nhóm Thông tư thuế về nhánh chính sách chung."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase38-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase38-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase38_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    if len(selected) != 50:
        raise RuntimeError(f"Phase38 cần đúng 50 bài, hiện chỉ chọn được {len(selected)}")
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase38-thongtu-generic-normalize",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase38Batch50"] = decision["reason"]
