#!/usr/bin/env python3
"""Chặng 10 - Batch 1: apply nhóm very-safe từ auto-safe queue."""
...
INPUT_PATH = ROOT / "docs" / "lv3-stage10-very-safe.apply.json"
OUT_JSON_PATH = ROOT / "docs" / "lv3-stage10-batch1-apply.json"
OUT_MD_PATH = ROOT / "docs" / "lv3-stage10-batch1-apply.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_stage10", module_path)
...
def main() -> None:
    ...
    lines = [
        "# Chặng 10 - Batch 1 apply (very-safe)",
        "",
        ...
        "## Danh sách applied",
        "",
        "| # | href | lv2 | lv3 | score | ov3 |",
        "|---:|---|---|---|---:|---:|",
    ]
    for i, r in enumerate(applied, 1):
        lines.append(
            f"| {i} | `{r['href']}` | `{r['topicLv2Key']}` | `{r['topicLv3Key']}` | {r['score']} | {r.get('ov3',0)} |"
        )
