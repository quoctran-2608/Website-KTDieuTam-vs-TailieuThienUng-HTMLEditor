#!/usr/bin/env python3
"""Phase 41: batch 50 - cân bằng Thông tư thuế + chốt tài liệu dạng bảng kê kế toán."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase41-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase41-batch50.md"
...
KE_TOAN_TOPUP = [
    "thu-vien/mau-bang-ke-chi-tiet-vat-tu-con-lai-cuoi-ky-theo-thong-tu-99.html",
    "thu-vien/mau-bang-ke-mua-hang-06-vt-theo-thong-tu-99.html",
    "thu-vien/mau-bang-ke-mua-lai-co-phieu-theo-qd-48-va-15.html",
    "thu-vien/mau-bang-ke-trich-nop-cac-khoan-theo-luong-10-ldtl.html",
    "thu-vien/mau-bang-ke-trich-nop-cac-khoan-theo-luong-theo-qd-48-va-15.html",
    "thu-vien/mau-bang-ke-trich-nop-cac-khoan-theo-luong-theo-thong-tu-99.html",
]


def load_importer_module():
    module_path = ROOT / "tools" / "import_stage1_20.py"
    spec = importlib.util.spec_from_file_location("importer_phase41_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        if cur != "thong-tu-chinh-sach-chung":
            ...
            if any(k in title for k in ["gtgt", "hóa đơn", "hoá đơn"]):
                tgt = "thong-tu-gtgt-hoa-don"
            elif any(k in title for k in ["tndn", "thu nhập doanh nghiệp"]):
                tgt = "thong-tu-tndn"
            elif any(k in title for k in ["kê khai", "quản lý thuế", "đăng ký thuế", "mst", "mã số thuế"]):
        ...
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                ...
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase41-thong-tu-balance",
            }

    # B) 6 bài top-up kế toán dạng bảng kê tài liệu
    for href in KE_TOAN_TOPUP:
        decisions[href] = {
            "kind": "bieu-mau",
            "lv1": "ke-toan",
            "lv2": "mau-bieu-ke-toan",
            "lv3": "mau-so-sach-ke-toan",
            "reason": "phase41-ke-toan-doc-type-final",
        }

    if len(decisions) != 50:
        raise RuntimeError(f"Phase41 cần đúng 50 bài, hiện build được {len(decisions)}")
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase41Batch50"] = decision["reason"]
