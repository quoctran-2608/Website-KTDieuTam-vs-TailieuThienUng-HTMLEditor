#!/usr/bin/env python3
"""Phase 31: batch 50 - refine từ 'mẫu bảng kê/phụ lục/hồ sơ' về nhánh thuế chuyên biệt."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase31-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase31-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase31_batch50", module_path)
...
def target_lv3_from_title(title: str) -> str | None:
    t = title.lower()
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    refinable = []
    for a in rows:
        if not pattern.search(a["title"]):
            continue
        tgt = target_lv3_from_title(a["title"])
        if tgt and tgt != "mau-bang-ke-phu-luc-ho-so":
            refinable.append((a["href"], tgt))

    selected = sorted(refinable, key=lambda x: x[0])[:50]
    if len(selected) != 50:
        raise RuntimeError(f"Phase31 cần đúng 50 bài, hiện chọn được {len(selected)}")
    ...
    for href, lv3 in selected:
        decisions[href] = {
            ...
            "lv2": "mau-bieu-thue",
            "lv3": lv3,
            "reason": "phase31-thue-form-refine-from-bangke",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase31Batch50"] = decision["reason"]
