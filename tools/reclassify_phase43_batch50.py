#!/usr/bin/env python3
"""Phase 43: batch 50 - document-type normalize mẫu thuế pattern về bucket phụ lục."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase43-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase43-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase43_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    selected = sorted(rows, key=lambda x: x["href"])[:50]
    if len(selected) != 50:
        raise RuntimeError(f"Phase43 cần đúng 50 bài, hiện chỉ chọn được {len(selected)}")
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase43-document-type-normalize-bangke-phuluc",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase43Batch50"] = decision["reason"]
