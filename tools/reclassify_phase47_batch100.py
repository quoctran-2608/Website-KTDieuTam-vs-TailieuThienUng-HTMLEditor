#!/usr/bin/env python3
"""Phase 47: batch 100 - mixed non-toggle cleanup across tax forms / thông tư / nghị định / công văn."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase47-batch100.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase47-batch100.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase47_batch100", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    # A) 44 mẫu thuế generic -> specific
    tax_rows = [
        ...
        and (a.get("topicLv3Key") or "") == "mau-bang-ke-phu-luc-ho-so"
    ]
    picked = 0
    for a in tax_rows:
        tgt = tax_spec(a["title"])
        if tgt:
            decisions[a["href"]] = {
                "kind": "bieu-mau",
                "lv1": "thue",
                "lv2": "mau-bieu-thue",
                "lv3": tgt,
                "reason": "phase47-thue-generic-to-specific",
            }
            picked += 1
            if picked >= 44:
                break

    # B) 35 thông tư specific -> generic
    tt_rows = [
        a for a in sorted(data_articles, key=lambda x: x["href"])
        if a.get("section") == "thu-vien"
    ...
    for a in tt_rows:
        decisions[a["href"]] = {
            ...
            "lv1": "thue",
            "lv2": "thong-tu",
            "lv3": "thong-tu-chinh-sach-chung",
            "reason": "phase47-thongtu-specific-to-generic",
        }

    # C) 16 nghị định specific -> generic
    nd_rows = [
        a for a in sorted(data_articles, key=lambda x: x["href"])
        if a.get("section") == "thu-vien"
        and a.get("libraryKindKey") == "van-ban"
        ...
        and (a.get("topicLv3Key") or "") == "nghi-dinh-quan-ly-xu-phat"
    ]
    for a in nd_rows:
        decisions[a["href"]] = {
            "kind": "van-ban",
            "lv1": "thue",
            "lv2": "nghi-dinh",
            "lv3": "nghi-dinh-chinh-sach-chung",
            "reason": "phase47-nghidinh-specific-to-generic",
        }

    # D) 5 công văn generic -> kê khai
    cv_rows = [
        a for a in sorted(data_articles, key=lambda x: x["href"])
        if a.get("section") == "thu-vien"
        and a.get("libraryKindKey") == "van-ban"
        ...
        and re.search(r"kê khai|quản lý thuế|đăng ký thuế|mst|mã số thuế", a["title"], re.IGNORECASE)
    ]
    for a in cv_rows[:5]:
        decisions[a["href"]] = {
            "kind": "van-ban",
            "lv1": "thue",
            "lv2": "cong-van",
            "lv3": "cong-van-ke-khai-quan-ly-thue",
            "reason": "phase47-congvan-generic-to-specific",
        }

    if len(decisions) != 100:
        raise RuntimeError(f"Phase47 cần đúng 100 bài, hiện build được {len(decisions)}")
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase47Batch100"] = decision["reason"]
