#!/usr/bin/env python3
"""Phase 5: rà và sửa nhóm Hướng dẫn > Thuế > Thuế thu nhập cá nhân."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase5-huong-dan-thue-tncn.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase5-huong-dan-thue-tncn.md"
...
DECISIONS: Dict[str, Dict[str, str]] = {
    # TNCN -> TNDN (sai chủ đề thuế đích)
    "thu-vien/cach-hach-toan-thue-tncn-phai-nop-khi-tam-tinh-quyet-toan.html": {
        "kind": "huong-dan",
        "lv1": "thue",
        "lv2": "tndn",
        "lv3": "hach-toan-tndn",
        "reason": "title-noi-dung-ve-thue-tndn",
    },
    # TNCN -> Kế toán tài khoản
    "thu-vien/hach-toan-cac-khoan-thue-phai-nop-cho-nha-nuoc-tk-333.html": {
        "kind": "huong-dan",
        "lv1": "ke-toan",
        "lv2": "tai-khoan-hach-toan",
        "lv3": "thue-nghia-vu",
        "reason": "tk333-ke-toan-tai-khoan",
    },
    # TNCN -> Mã số thuế
    "thu-vien/thu-tuc-dang-ky-ma-so-thue-thu-nhap-ca-nhan-moi-nhat.html": {
        "kind": "huong-dan",
        "lv1": "thue",
        "lv2": "ma-so-thue-dang-ky-thue",
        "lv3": "mst-dang-ky-lan-dau-doi-tuong",
        "reason": "dang-ky-mst-ca-nhan",
    },
    # TNCN -> Văn bản/Nghị định quản lý xử phạt
    "thu-vien/nghi-dinh-52-2021-nd-cp-gia-han-thoi-han-nop-thue-va-tien-thue-dat.html": {
        "kind": "van-ban",
        "lv1": "thue",
        "lv2": "nghi-dinh",
        "lv3": "nghi-dinh-quan-ly-xu-phat",
        "reason": "nghi-dinh-gia-han-nop-thue",
    },
    "thu-vien/nghi-dinh-82-moi-nhat-2025-gia-han-thoi-gian-nop-thue-va-tien-thue-dat.html": {
        "kind": "van-ban",
        "lv1": "thue",
        "lv2": "nghi-dinh",
        "lv3": "nghi-dinh-quan-ly-xu-phat",
    ...
    # TNCN -> GTGT (chủ đề liên thuế chung, đang có precedent ở GTGT/lập xử lý)
    "thu-vien/muc-phat-cham-nop-tien-thue-gtgt-tncn-tndn.html": {
        "kind": "huong-dan",
        "lv1": "thue",
        "lv2": "gtgt-hoa-don",
        "lv3": "lap-xu-ly-hoa-don",
        "reason": "bai-phat-cham-nop-thue-da-sac-thue",
    },
    "thu-vien/cac-bao-cao-thue-hang-thang-phai-nop-cho-co-quan-thue.html": {
        "kind": "huong-dan",
        "lv1": "thue",
        "lv2": "gtgt-hoa-don",
        "lv3": "bao-cao-bang-ke",
        "reason": "tong-hop-bao-cao-thue-da-sac-thue",
    },
}
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase5_huongdan_thue_tncn", module_path)
...
def main() -> None:
    ...
    for href, decision in DECISIONS.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase5ThuVienThueTNCN"] = decision["reason"]
    ...
    phase_bucket_after = [
        ...
        and a.get("topicLv2Key") == "tncn"
    ]

    payload = {
        "generatedAt": datetime.now().isoformat(),
        "phase": "phase5-huong-dan-thue-tncn",
        ...
        "deferredManualReview": [
            {
                "href": "thu-vien/ca-ch-lam-bao-quye-t-toa-n-thue-tncn-nam-2016.html",
                "reason": "legacy-year-content-but-still-tncn-topic",
            }
        ],
    }
    ...
    lines = [
        "# Phase 5 - Rà soát Hướng dẫn / Thuế / Thuế thu nhập cá nhân",
        "",
        f"- Thời gian chạy: `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`",
        f"- Applied: **{len(applied)}**",
        f"- Skipped: **{len(skipped)}**",
        f"- Còn lại trong bucket sau phase 5: **{len(phase_bucket_after)}**",
    ...
    lines += [
        ...
        "",
        "- Một số bài TNCN theo năm cũ (2013/2015/2016) vẫn đúng chủ đề, chưa move trong phase này.",
    ]
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
