#!/usr/bin/env python3
"""Phase 48: batch 100 - mixed non-toggle set theo preflight (A44+B35+C16+D5)."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase48-batch100.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase48-batch100.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase48_batch100", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    # A) 54 tax pattern specific -> generic
    ...
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        raw[a["href"]] = {
            ...
            "reason": "phase48-thue-pattern-to-generic",
        }

    # B) 44 thong-tu balance with existing keys only
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            a.get("section") == "thu-vien"
        ...
        if tgt and tgt != cur:
            raw[a["href"]] = {
                ...
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase48-thong-tu-balance",
            }

    # C) top-up 2 nghi-dinh candidates
    nd_rows = [
    ...
    for a in nd_rows[:2]:
        ...
        if tgt != cur:
            raw[a["href"]] = {
                ...
                "lv2": "nghi-dinh",
                "lv3": tgt,
                "reason": "phase48-nghidinh-topup",
            }

    if len(raw) < 100:
        raise RuntimeError(f"Phase48 cần >=100 candidates, hiện build được {len(raw)}")
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase48Batch100"] = decision["reason"]
