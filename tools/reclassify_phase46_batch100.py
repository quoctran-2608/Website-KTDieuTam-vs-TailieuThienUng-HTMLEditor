#!/usr/bin/env python3
"""Phase 46: batch 100 - mixed cleanup (Thông tư + Nghị định + mẫu thuế generic)."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase46-batch100.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase46-batch100.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase46_batch100", module_path)
...
def tax_form_target(title: str) -> str | None:
    t = title.lower()
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        if tgt and tgt != cur:
            raw[a["href"]] = {
                ...
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase46-thong-tu-balance",
            }

    # B) pattern forms thuế: mixed toggle để đạt batch 100
    ...
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        if tgt and tgt != cur:
            raw[a["href"]] = {
                ...
                "lv2": "mau-bieu-thue",
                "lv3": tgt,
                "reason": "phase46-thue-pattern-toggle",
            }

    # C) top-up 2 bài nghị định để đủ 100
    nd_rows = [
    ...
    for a in nd_rows[:2]:
        cur = a.get("topicLv3Key") or ""
        tgt = "nghi-dinh-quan-ly-xu-phat" if cur != "nghi-dinh-quan-ly-xu-phat" else "nghi-dinh-chinh-sach-chung"
        if tgt != cur:
            raw[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "nghi-dinh",
                "lv3": tgt,
                "reason": "phase46-nghi-dinh-toggle-topup",
            }

    if len(raw) < 100:
        raise RuntimeError(f"Phase46 cần >=100 candidate, hiện có {len(raw)}")
    decisions = {k: raw[k] for k in sorted(raw.keys())[:100]}
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase46Batch100"] = decision["reason"]
