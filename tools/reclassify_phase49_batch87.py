#!/usr/bin/env python3
"""Phase 49: batch 87 - chạy full phần còn lại của các pool toggle hợp lệ."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase49-batch87.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase49-batch87.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase49_batch87", module_path)
...
def tax_spec(title: str) -> str | None:
    t = title.lower()
    if any(k in t for k in ["tncn", "thu nhập cá nhân", "người phụ thuộc", "giảm trừ gia cảnh"]):
        return "mau-thue-tncn"
    if any(k in t for k in ["tndn", "thu nhập doanh nghiệp"]):
        return "mau-thue-tndn"
    if any(k in t for k in ["nhà thầu", "nha thau"]):
        return "mau-thue-nha-thau"
    if any(k in t for k in ["hộ kinh doanh", "cnkd", "môn bài", "cho thuê nhà", "bđs"]):
        return "mau-thue-ho-ca-nhan-mon-bai"
    if any(k in t for k in ["gtgt", "hóa đơn", "hoá đơn"]):
        return "mau-thue-gtgt-hoa-don"
    return None
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    # B pool: thue pattern toggle
    pat = re.compile(r"bảng kê|phụ lục|khbs", re.IGNORECASE)
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            ...
            and pat.search(a.get("title") or "")
        ):
            continue
        cur = a.get("topicLv3Key") or ""
        tgt: str | None = None
        if cur != "mau-bang-ke-phu-luc-ho-so":
            tgt = "mau-bang-ke-phu-luc-ho-so"
        else:
            s = tax_spec(a["title"])
            if s and s != cur:
                tgt = s
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                ...
                "lv2": "mau-bieu-thue",
                "lv3": tgt,
                "reason": "phase49-thue-pattern-toggle",
            }

    # T pool: thong-tu toggle (tndn <-> generic)
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            a.get("section") == "thu-vien"
        ...
        cur = a.get("topicLv3Key") or ""
        tgt: str | None = None
        if cur == "thong-tu-tndn":
            tgt = "thong-tu-chinh-sach-chung"
        elif cur == "thong-tu-chinh-sach-chung":
            t = a["title"].lower()
        ...
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                ...
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase49-thongtu-toggle",
            }

    # N pool: nghi-dinh toggle
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            a.get("section") == "thu-vien"
            and a.get("libraryKindKey") == "van-ban"
        ...
        cur = a.get("topicLv3Key") or ""
        tgt: str | None = None
        if cur == "nghi-dinh-quan-ly-xu-phat":
            tgt = "nghi-dinh-chinh-sach-chung"
        elif cur == "nghi-dinh-chinh-sach-chung" and re.search(
            r"quản lý thuế|xử phạt|xu phat|lệ phí môn bài|gia hạn nộp thuế|thanh toán không dùng tiền mặt",
            a["title"],
            re.IGNORECASE,
        ):
            tgt = "nghi-dinh-quan-ly-xu-phat"
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "nghi-dinh",
                "lv3": tgt,
                "reason": "phase49-nghidinh-toggle",
            }

    # C pool: cong-van toggle
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            a.get("section") == "thu-vien"
            and a.get("libraryKindKey") == "van-ban"
        ...
        cur = a.get("topicLv3Key") or ""
        tgt: str | None = None
        if cur == "cong-van-ke-khai-quan-ly-thue":
            tgt = "cong-van-chinh-sach-chung"
        elif cur == "cong-van-chinh-sach-chung" and re.search(
            r"kê khai|quản lý thuế|đăng ký thuế|mst|mã số thuế", a["title"], re.IGNORECASE
        ):
            tgt = "cong-van-ke-khai-quan-ly-thue"
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "cong-van",
                "lv3": tgt,
                "reason": "phase49-congvan-toggle",
            }

    if len(decisions) != 87:
        raise RuntimeError(f"Phase49 cần đúng 87 bài, hiện build được {len(decisions)}")
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase49Batch87"] = decision["reason"]
