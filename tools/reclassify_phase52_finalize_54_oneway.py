#!/usr/bin/env python3
"""Phase 52: finalize 54 rows with one-way rule for bang-ke/phu-luc/khbs."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase52-finalize54-oneway.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase52-finalize54-oneway.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase52_finalize54", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    selected = sorted(rows, key=lambda x: x["href"])
    if len(selected) != 54:
        raise RuntimeError(f"Phase52 expected 54 candidates, found {len(selected)}")
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase52-final-oneway-bangke-phuluc",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase52Finalize54"] = decision["reason"]
    ...
    payload = {
        ...
        "phase": "phase52-finalize54-oneway",
        "plannedCount": 54,
