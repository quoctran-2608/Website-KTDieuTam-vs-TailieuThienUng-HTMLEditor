#!/usr/bin/env python3
"""Phase 50 (one-way): batch 50 - chỉ generic -> specific cho thue/thong-tu/nghi-dinh/cong-van."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase50-batch50-oneway.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase50-batch50-oneway.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase50_batch50_oneway", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    # B) van-ban/thue/thong-tu generic -> specific (43)
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        if tgt:
            decisions[a["href"]] = {
                ...
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase50-thongtu-generic-to-specific",
            }

    # C) van-ban/thue/nghi-dinh generic -> quan-ly-xu-phat (2)
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            a.get("section") == "thu-vien"
            and a.get("libraryKindKey") == "van-ban"
            ...
            continue
        if re.search(r"quản lý thuế|xử phạt|xu phat|lệ phí môn bài|gia hạn nộp thuế|thanh toán không dùng tiền mặt", a["title"], re.IGNORECASE):
            decisions[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "nghi-dinh",
                "lv3": "nghi-dinh-quan-ly-xu-phat",
                "reason": "phase50-nghidinh-generic-to-quanly",
            }

    # D) van-ban/thue/cong-van generic -> ke-khai (5)
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            a.get("section") == "thu-vien"
            and a.get("libraryKindKey") == "van-ban"
            ...
            continue
        if re.search(r"kê khai|quản lý thuế|đăng ký thuế|mst|mã số thuế", a["title"], re.IGNORECASE):
            decisions[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "cong-van",
                "lv3": "cong-van-ke-khai-quan-ly-thue",
                "reason": "phase50-congvan-generic-to-kekhai",
            }
    if len(decisions) != 50:
        raise RuntimeError(f"Phase50 (one-way) cần đúng 50 bài, hiện build được {len(decisions)}")
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase50Batch50OneWay"] = decision["reason"]
