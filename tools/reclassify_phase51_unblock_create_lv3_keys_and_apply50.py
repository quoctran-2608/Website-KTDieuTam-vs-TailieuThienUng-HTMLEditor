#!/usr/bin/env python3
"""Phase 51 (unblock): tạo key lv3 thiếu bằng seed + apply 50 one-way candidates."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase51-unblock-apply50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase51-unblock-apply50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase51_unblock50", module_path)
...
def ensure_missing_lv3_keys(data_articles: List[Dict]) -> Dict[str, str]:
    """Create missing lv3 keys by seeding labels from authoritative naming."""
    lv3_map = {a.get("topicLv3Key"): a.get("topicLv3Label") for a in data_articles if a.get("topicLv3Key")}
    needed = {
        "thong-tu-gtgt-hoa-don": "Thông tư GTGT/Hóa đơn",
        "thong-tu-ke-khai-quan-ly-thue": "Thông tư Kê khai/Quản lý thuế",
        "thong-tu-tncn": "Thông tư TNCN",
        "thong-tu-ho-ca-nhan-kinh-doanh": "Thông tư Hộ/Cá nhân kinh doanh",
        "cong-van-tndn": "Công văn TNDN",
    }
    created: Dict[str, str] = {}
    for k, label in needed.items():
        if k not in lv3_map:
            created[k] = label
    return created


def build_decisions(data_articles: List[Dict], missing_lv3: Dict[str, str]) -> Dict[str, Dict[str, str]]:
    decisions: Dict[str, Dict[str, str]] = {}

    # A) 43 thong-tu generic -> specific
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            ...
            and (a.get("topicLv3Key") or "") == "thong-tu-chinh-sach-chung"
        ):
            continue
        t = a["title"].lower()
        tgt: str | None = None
        ...
        elif any(k in t for k in ["tncn", "thu nhập cá nhân"]):
            tgt = "thong-tu-tncn"
        elif any(k in t for k in ["hộ kinh doanh", "cá nhân kinh doanh", "cnkd"]):
            tgt = "thong-tu-ho-ca-nhan-kinh-doanh"
        if tgt:
            decisions[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase51-thongtu-generic-to-specific",
            }

    # B) 4 cong-van generic -> tndn
    for a in sorted(data_articles, key=lambda x: x["href"]):
        if not (
            a.get("section") == "thu-vien"
            and a.get("libraryKindKey") == "van-ban"
            and a.get("topicLv1Key") == "thue"
            and a.get("topicLv2Key") == "cong-van"
            and (a.get("topicLv3Key") or "") == "cong-van-chinh-sach-chung"
        ):
            continue
        t = a["title"].lower()
        if "tndn" in t or "thu nhập doanh nghiệp" in t:
            decisions[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "cong-van",
                "lv3": "cong-van-tndn",
                "reason": "phase51-congvan-generic-to-tndn",
            }

    # C) 3 thong-tu generic -> tncn
    # (already covered by A logic when title has tncn keywords)

    if not decisions:
        raise RuntimeError("Phase51 không tìm thấy candidate one-way hợp lệ")
...
def main() -> None:
    ...
    # Unblock: declare missing lv3 keys
    created_lv3 = ensure_missing_lv3_keys(data_articles)
    lv3_labels.update(created_lv3)

    decisions = build_decisions(data_articles, created_lv3)
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase51UnblockApply"] = decision["reason"]
    ...
    payload = {
        ...
        "phase": "phase51-unblock-apply50",
        "plannedCount": planned_count,
        "appliedCount": len(applied),
        "skippedCount": len(skipped),
        "createdLv3Keys": created_lv3,
        "applied": applied,
        "skipped": skipped,
    ...
    lines = [
        "# Phase 51 - Unblock + Apply 47 (one-way)",
        "",
    ...
    for k, v in sorted(created_lv3.items()):
        lines.append(f"- `{k}`: {v}")
    lines += [
        "",
        "## Các bài đã chỉnh",
        ...
        json.dumps(
            {
                ...
                "applied": len(applied),
                "skipped": len(skipped),
                "created_lv3_keys": len(created_lv3),
