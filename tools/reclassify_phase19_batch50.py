#!/usr/bin/env python3
"""Phase 19: batch 50 - tiếp tục chuyển release HTKK từ Biểu mẫu sang Công cụ."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase19-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase19-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase19_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    src_rows = [
        ...
        and a.get("topicLv2Key") == "htkk-etax-thue-dien-tu"
    ]
    pattern = re.compile(r"phần mềm\s*(hỗ\s*trợ\s*kê\s*khai\s*thuế\s*)?htkk", re.IGNORECASE)
    matched = [a for a in sorted(src_rows, key=lambda x: x["href"]) if pattern.search(a.get("title") or "")]

    # phase19 chạy trên trạng thái sau phase18 => lấy 50 bài đầu còn lại
    selected = matched[:50]
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase19-htkk-releases-to-cong-cu",
        }
    if len(decisions) != 50:
        raise RuntimeError(f"Phase19 cần đúng 50 bài, hiện chỉ chọn được {len(decisions)}")
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase19Batch50"] = decision["reason"]
