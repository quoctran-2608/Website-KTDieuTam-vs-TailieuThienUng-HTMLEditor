#!/usr/bin/env python3
"""Phase 18: batch 50 - chuyển nhóm release HTKK từ Biểu mẫu sang Công cụ."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase18-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase18-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase18_batch50", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    for a in selected:
        decisions[a["href"]] = {
            ...
            "reason": "phase18-htkk-releases-to-cong-cu",
        }
    if len(decisions) != 50:
        raise RuntimeError(f"Phase18 cần đúng 50 bài, hiện chỉ chọn được {len(decisions)}")
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase18Batch50"] = decision["reason"]
