#!/usr/bin/env python3
"""Phase 45: batch 100 - mixed cleanup (Thông tư balance + Nghị định quản lý + Thuế generic->specific)."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase45-batch100.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase45-batch100.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase45_batch100", module_path)
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    # A) 44 bài cân bằng Thông tư thuế
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                ...
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase45-thong-tu-balance",
            }

    # B) 12 bài nghị định theo từ khóa quản lý/xử phạt
    nd_rows = [
    ...
    for a in nd_rows[:12]:
        decisions[a["href"]] = {
            ...
            "reason": "phase45-nghi-dinh-quanly",
        }

    # C) 44 bài mẫu thuế generic -> specific (take first 44)
    tax_generic = [
        a for a in sorted(data_articles, key=lambda x: x["href"])
        if a.get("section") == "thu-vien"
    ...
    picked = 0
    for a in tax_generic:
        tgt = tax_form_target(a["title"])
        if tgt and tgt != "mau-bang-ke-phu-luc-ho-so":
            decisions[a["href"]] = {
                "kind": "bieu-mau",
                "lv1": "thue",
                "lv2": "mau-bieu-thue",
                "lv3": tgt,
                "reason": "phase45-thue-generic-refine",
            }
            picked += 1
            if picked >= 44:
                break
    ...
    if len(decisions) != 100:
        raise RuntimeError(f"Phase45 cần đúng 100 bài, hiện build được {len(decisions)}")
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase45Batch100"] = decision["reason"]
