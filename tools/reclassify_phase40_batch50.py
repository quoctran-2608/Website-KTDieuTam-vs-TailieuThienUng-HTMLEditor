#!/usr/bin/env python3
"""Phase 40: batch 50 - refine Thông tư thuế từ generic về nhánh chuyên biệt."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase40-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase40-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase40_batch50", module_path)
...
def thong_tu_target_lv3_from_generic(title: str) -> str | None:
    """Map generic thong-tu to existing specific keys only."""
    s = title.lower()
    ...
    if any(k in s for k in ["kê khai", "quản lý thuế", "đăng ký thuế", "mst", "mã số thuế"]):
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    # Group A: cân bằng trong bucket Thông tư (44 bài)
    for a in sorted(data_articles, key=lambda x: x["href"]):
        ...
        if cur != "thong-tu-chinh-sach-chung":
            ...
            tgt = thong_tu_target_lv3_from_generic(a["title"])
        if tgt and tgt != cur:
            decisions[a["href"]] = {
                "kind": "van-ban",
                "lv1": "thue",
                "lv2": "thong-tu",
                "lv3": tgt,
                "reason": "phase40-thong-tu-balance",
            }

    # Group B: top-up 6 bài công văn non-generic -> generic
    cv_rows = [
    ...
    for a in cv_rows[:6]:
        decisions[a["href"]] = {
            ...
            "lv1": "thue",
            "lv2": "cong-van",
            "lv3": "cong-van-chinh-sach-chung",
            "reason": "phase40-cong-van-generic-finish",
        }

    if len(decisions) != 50:
        raise RuntimeError(f"Phase40 cần đúng 50 bài, hiện build được {len(decisions)}")

    # ensure deterministic order in report/application
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase40Batch50"] = decision["reason"]
