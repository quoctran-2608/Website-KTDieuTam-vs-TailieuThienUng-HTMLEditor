#!/usr/bin/env python3
"""Phase 39: batch 50 - refine từ bucket phụ lục chung về nhánh thuế chuyên biệt."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase39-batch50.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase39-batch50.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase39_batch50", module_path)
...
def target_lv3(title: str) -> str | None:
    t = title.lower()
...
def build_decisions(data_articles: List[Dict]) -> Dict[str, Dict[str, str]]:
    ...
    cand = []
    for a in rows:
        tgt = target_lv3(a["title"])
        if tgt:
            cand.append((a["href"], tgt))
    selected = sorted(cand, key=lambda x: x[0])[:50]
    if len(selected) != 50:
        raise RuntimeError(f"Phase39 cần đúng 50 bài, hiện chỉ chọn được {len(selected)}")
    ...
    for href, lv3 in selected:
        decisions[href] = {
            ...
            "lv2": "mau-bieu-thue",
            "lv3": lv3,
            "reason": "phase39-thue-specific-refine-from-generic",
        }
    return decisions
...
def main() -> None:
    ...
    for href, decision in decisions.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase39Batch50"] = decision["reason"]
