#!/usr/bin/env python3
"""Phase 8: rà và sửa nhóm Hướng dẫn > Kế toán > Nghiệp vụ theo phần hành."""
...
OUT_JSON = ROOT / "docs" / "thu-vien-phase8-huong-dan-ke-toan-nghiep-vu.json"
OUT_MD = ROOT / "docs" / "thu-vien-phase8-huong-dan-ke-toan-nghiep-vu.md"
...
DECISIONS: Dict[str, Dict[str, str]] = {
    "thu-vien/cach-lap-so-chi-tiet-ban-hang-mau-s35-dn-theo-thong-tu-200.html": {
        "kind": "huong-dan",
        "lv1": "ke-toan",
        "lv2": "chung-tu-so-sach",
        "lv3": "so-sach-tien-kho-chi-tiet",
        "reason": "mau-so-chi-tiet-ban-hang-s35",
    },
    "thu-vien/cach-lap-the-tinh-gia-thanh-san-pham-dich-vu-mau-s37-dn.html": {
        "kind": "huong-dan",
        "lv1": "ke-toan",
        "lv2": "chung-tu-so-sach",
        "lv3": "so-sach-gia-thanh-ban-hang",
        "reason": "mau-the-gia-thanh-s37",
    },
}
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_phase8_huongdan_ketoan_nghiepvu", module_path)
...
def main() -> None:
    ...
    for href, decision in DECISIONS.items():
        ...
        if isinstance(article.get("classificationReasons"), dict):
            article["classificationReasons"]["phase8ThuVienKeToanNghiepVu"] = decision["reason"]
    ...
    phase_bucket_after = [
        ...
        and a.get("topicLv2Key") == "nghiep-vu-theo-phan-hanh"
    ]

    payload = {
        "generatedAt": datetime.now().isoformat(),
        "phase": "phase8-huong-dan-ke-toan-nghiep-vu-theo-phan-hanh",
    ...
    lines = [
        "# Phase 8 - Rà soát Hướng dẫn / Kế toán / Nghiệp vụ theo phần hành",
        "",
        f"- Thời gian chạy: `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`",
        f"- Applied: **{len(applied)}**",
        f"- Skipped: **{len(skipped)}**",
        f"- Còn lại trong bucket sau phase 8: **{len(phase_bucket_after)}**",
