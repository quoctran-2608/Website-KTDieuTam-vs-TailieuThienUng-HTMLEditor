#!/usr/bin/env python3
"""Chặng 7 - Apply batch lớn an toàn từ stage6 auto candidates."""
...
APPLY_INPUT_PATH = ROOT / "docs" / "lv3-stage7-batch1-candidates.apply.json"
APPLY_JSON_PATH = ROOT / "docs" / "lv3-stage7-batch1-apply.json"
APPLY_MD_PATH = ROOT / "docs" / "lv3-stage7-batch1-apply.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_stage7", module_path)
...
def main() -> None:
    ...
    apply_payload = json.loads(APPLY_INPUT_PATH.read_text(encoding="utf-8"))
    candidates = apply_payload.get("records", [])
    excluded_seed = apply_payload.get("excluded", [])
    ...
    for row in candidates:
        ...
        if not article:
            skipped.append({"href": href, "reason": "missing-in-data-articles"})
            continue
        if article.get("section") != "thu-vien":
            skipped.append({"href": href, "reason": "not-thu-vien"})
            continue
        if not (article.get("topicLv2Key") or "").strip():
            skipped.append({"href": href, "reason": "missing-topicLv2"})
            continue
        old_lv3 = (article.get("topicLv3Key") or "").strip()
        if old_lv3:
            skipped.append({"href": href, "reason": f"already-has-lv3:{old_lv3}"})
            continue
        article["topicLv3Key"] = lv3_key
        article["topicLv3Label"] = labels.get(lv3_key) or row.get("suggestedTopicLv3Label") or ""
        applied.append(
            {
                "href": href,
                ...
                "topicLv3Label": article.get("topicLv3Label") or "",
    ...
    APPLY_JSON_PATH.write_text(json.dumps(out, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    for i, row in enumerate(applied, 1):
        lines.append(
            f"| {i} | `{row['href']}` | `{row['topicLv2Key']}` | `{row['topicLv3Key']}` | `{row['confidence']}` | {row['score']} |"
        )
