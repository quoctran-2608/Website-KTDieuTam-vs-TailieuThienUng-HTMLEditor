#!/usr/bin/env python3
"""Chặng 11 - Batch 1 apply nhóm rất chắc từ auto-safe còn lại."""
...
INPUT_PATH = ROOT / "docs" / "lv3-stage11-batch1-candidates.apply.json"
OUT_JSON_PATH = ROOT / "docs" / "lv3-stage11-batch1-apply.json"
OUT_MD_PATH = ROOT / "docs" / "lv3-stage11-batch1-apply.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_stage11", module_path)
...
def main() -> None:
    ...
    lines = [
        "# Chặng 11 - Batch 1 apply (very-safe from remaining auto-safe)",
        "",
    ...
    for i, r in enumerate(applied, 1):
        lines.append(f"| {i} | `{r['href']}` | `{r['topicLv2Key']}` | `{r['topicLv3Key']}` | {r['score']} |")
