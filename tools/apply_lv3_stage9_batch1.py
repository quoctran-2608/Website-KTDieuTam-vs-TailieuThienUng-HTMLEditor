#!/usr/bin/env python3
"""Chặng 9 - Batch 1 apply từ queue ưu tiên cao (safe vetted)."""
...
INPUT_PATH = ROOT / "docs" / "lv3-stage9-batch1-candidates.apply.json"
OUT_JSON_PATH = ROOT / "docs" / "lv3-stage9-batch1-apply.json"
OUT_MD_PATH = ROOT / "docs" / "lv3-stage9-batch1-apply.md"
QUEUE_MD_PATH = ROOT / "docs" / "lv3-stage9-priority-queues.md"
QUEUE_JSON_PATH = ROOT / "docs" / "lv3-stage9-priority-queues.json"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_stage9", module_path)
...
def write_priority_queues(remaining_rows: List[Dict]) -> None:
    auto_safe = []
    review_medium = []
    taxonomy_first = []
    for r in remaining_rows:
        conf = r.get("confidence")
        lv2 = r.get("topicLv2Key") or ""
        if conf in {"high", "auto-single-leaf"} and lv2 not in {"luat-bo-luat", "nghi-quyet-quyet-dinh"}:
            auto_safe.append(r)
        elif conf == "medium":
            review_medium.append(r)
        else:
            taxonomy_first.append(r)

    payload = {
        "generatedAt": datetime.now().isoformat(),
        "summary": {
            "autoSafe": len(auto_safe),
            "reviewMedium": len(review_medium),
            "taxonomyFirst": len(taxonomy_first),
        },
        "autoSafe": auto_safe,
        "reviewMedium": review_medium,
        "taxonomyFirst": taxonomy_first,
    }
    QUEUE_JSON_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Chặng 9 - Priority queues cho phần LV3 còn lại",
        "",
        f"- Thời gian chạy: `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`",
        f"- Auto-safe: **{len(auto_safe)}**",
        f"- Review-medium: **{len(review_medium)}**",
        f"- Taxonomy-first / low-none: **{len(taxonomy_first)}**",
        "",
        "## 20 dòng Auto-safe mẫu",
        "",
        "| # | href | lv2 | gợi ý lv3 | confidence | score |",
        "|---:|---|---|---|---|---:|",
    ]
    for i, r in enumerate(auto_safe[:20], 1):
        lines.append(
            f"| {i} | `{r['href']}` | `{r.get('topicLv2Key','')}` | `{r.get('suggestedTopicLv3Key','')}` | `{r.get('confidence','')}` | {r.get('score',0)} |"
        )
    lines += [
        "",
        "## 20 dòng Review-medium mẫu",
        "",
        "| # | href | lv2 | gợi ý lv3 | score | second |",
        "|---:|---|---|---|---:|---:|",
    ]
    for i, r in enumerate(review_medium[:20], 1):
        lines.append(
            f"| {i} | `{r['href']}` | `{r.get('topicLv2Key','')}` | `{r.get('suggestedTopicLv3Key','')}` | {r.get('score',0)} | {r.get('secondScore',0)} |"
        )
    lines += [
        "",
        "## 20 dòng Taxonomy-first/low mẫu",
        "",
        "| # | href | lv2 | gợi ý lv3 | confidence | score |",
        "|---:|---|---|---|---|---:|",
    ]
    for i, r in enumerate(taxonomy_first[:20], 1):
        lines.append(
            f"| {i} | `{r['href']}` | `{r.get('topicLv2Key','')}` | `{r.get('suggestedTopicLv3Key','')}` | `{r.get('confidence','')}` | {r.get('score',0)} |"
        )
...
def main() -> None:
    ...
    # refresh stage6 dryrun to build priority queues snapshot after apply
    subprocess.run(
        [sys.executable, str(ROOT / "tools" / "lv3_stage6_dryrun_remaining.py")],
        cwd=str(ROOT),
        check=True,
    ...
    stage6 = json.loads((ROOT / "docs" / "lv3-stage6-dryrun-remaining.json").read_text(encoding="utf-8"))
    remaining_rows = [r for r in stage6.get("records", []) if r.get("confidence") in {"high", "auto-single-leaf", "medium", "low", "none", "no-leaf-under-lv2"}]
    write_priority_queues(remaining_rows)
    ...
    payload = {
        ...
        "skippedRows": skipped,
        "priorityQueues": {
            "json": str(QUEUE_JSON_PATH.relative_to(ROOT)),
            "md": str(QUEUE_MD_PATH.relative_to(ROOT)),
        },
    }
    OUT_JSON_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# Chặng 9 - Batch 1 apply (safe vetted)",
        "",
        f"- Thời gian chạy: `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`",
    ...
    lines.append("")
    lines.append(f"- Queue snapshot: `{QUEUE_MD_PATH.relative_to(ROOT)}`")
        ...
        json.dumps(
            {
                ...
                "priorityQueues": str(QUEUE_MD_PATH.relative_to(ROOT)),
            },
            ensure_ascii=False,
            indent=2,
        )
