#!/usr/bin/env python3
"""Chặng 7 - Batch 2 apply nhỏ, an toàn cao."""
...
INPUT_PATH = ROOT / "docs" / "lv3-stage7-batch2-candidates.apply.json"
OUT_JSON_PATH = ROOT / "docs" / "lv3-stage7-batch2-apply.json"
OUT_MD_PATH = ROOT / "docs" / "lv3-stage7-batch2-apply.md"
...
def load_importer_module():
    ...
    spec = importlib.util.spec_from_file_location("importer_stage7b2", module_path)
...
def update_article_meta(path: Path, lv3_key: str, lv3_label: str) -> Tuple[bool, str]:
    if not path.exists():
...
def main() -> None:
    ...
    meta_skipped = []
    for row in applied:
        ok, reason = update_article_meta(ROOT / row["href"], row["topicLv3Key"], row["topicLv3Label"])
        if ok:
            meta_updated += 1
        else:
    ...
    lines = [
        "# Chặng 7 - Batch 2 apply (safe small set)",
        "",
